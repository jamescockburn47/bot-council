...
pub async fn run_round3(
    ...
    queries_phase1::insert_pairing(
        pool, debate_id, &pair1_a, &pair1_b, Some(&third_bot), &pairing_json,
    ...
    let pass_a_futures: Vec<_> = question_targets.iter().map(|(q_pseudo, t_pseudo)| {
    ...
    for ((q_pseudo, t_pseudo), (_, question_opt)) in question_targets.iter().zip(pass_a_results.iter()) {
