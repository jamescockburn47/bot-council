//! Legacy Round 3: two-pass cross-examination, used as the fallback when
//! the crux selector fails between R2 and R3.
//!
//! Preserved verbatim from the pre-crux implementation so adversarial
//! engagement still runs when MiniMax cannot select a single crux from
//! the R1 responses. Task 17 moved this out of `round3.rs` to keep the
//! crux-dispatch path under the 300-line file budget.
...
/// Run the legacy cross-examination R3: two passes over the MiniMax-
/// computed pairings.
/// Pass A: each bot poses a question to its partner (concurrent).
/// Pass B: each bot answers the question(s) posed to it (concurrent).
#[allow(clippy::too_many_arguments)]
pub async fn run_round3_cross_examination_legacy(
    pool: &SqlitePool,
