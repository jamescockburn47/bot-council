...
#[tokio::test]
async fn simple_mode_auto_approves_signed_in_submitter() {
    let (app, _pool) = common::test_app_simple_mode().await;
    let body = json!({
        "name": "SimpleModeBot",
        "endpoint_url": "http://localhost:9999/debate",
        "token": ""
...
#[tokio::test]
async fn simple_mode_accepts_http_url_and_missing_token() {
    let (app, pool) = common::test_app_simple_mode().await;
    let body = json!({
        "name": "NoTokenSimpleMode",
...
#[tokio::test]
async fn simple_mode_new_submission_archives_previous_from_same_owner_and_name() {
    let (app, pool) = common::test_app_simple_mode().await;

    let first = json!({
        "name": "SameBot",
        "endpoint_url": "http://first.example/debate",
        "token": ""
    });
    let first_req = Request::builder()
        ...
        .uri("/bots")
        .header("content-type", "application/json")
        .header("authorization", "Bearer owner-a")
    ...
    let second = json!({
        "name": "SameBot",
        "endpoint_url": "http://second.example/debate",
        "token": ""
    });
    ...
    let first_status: (String,) = sqlx::query_as("SELECT status FROM bots WHERE id = ?")
        .bind(&first_id)
        .fetch_one(&pool)
        .await
        .unwrap();
    let second_status: (String,) = sqlx::query_as("SELECT status FROM bots WHERE id = ?")
        .bind(&second_id)
        .fetch_one(&pool)
        .await
        .unwrap();

    assert_eq!(first_status.0, "inactive");
    assert_eq!(second_status.0, "active");
}

#[tokio::test]
async fn simple_mode_same_name_different_owner_keeps_both_active() {
    let (app, pool) = common::test_app_simple_mode().await;

    let first = json!({
        "name": "SharedName",
        "endpoint_url": "http://owner-a.example/debate",
        "token": ""
    });
    let first_req = Request::builder()
        .method("POST")
    ...
    let second = json!({
        "name": "SharedName",
        "endpoint_url": "http://owner-b.example/debate",
        "token": ""
    });
    let second_req = Request::builder()
        .method("POST")
    ...
    assert_eq!(first_status.0, "active");
    assert_eq!(second_status.0, "active");
}
