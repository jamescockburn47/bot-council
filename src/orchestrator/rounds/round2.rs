                    ...
                    if let Some(ref challenge) = resp.challenge {
                        ...
                        match validate_challenge(models_config, &challenge_json, &resp.response).await {
                            ...
                            Ok(v) => {
                                ...
                                match send_round2_request_with_timeout(client, &endpoint, &token, &req, timeout_secs).await {
                        ...
                        let reprompt = prompts::round2_reprompt(&topic, "No challenge object found in response");
                        let req = DebateRoundRequest {
                            session_id: debate_id.to_string(), round: 2,
                            role: role.as_str().to_string(),
                            context: round1_context.clone(), prompt: reprompt,
...
mod tests {
    ...
    use crate::bot_client::{build_http_client, DebateRoundRequest};
