...
/// Annotate an existing response row with an error-kind classification.
/// Intended to run immediately after `insert_response_full` on the
/// abstained path; safe to call repeatedly (last write wins).
pub async fn update_response_error(
    pool: &SqlitePool,
    response_id: &str,
    error_kind: &str,
    error_detail: &str,
    ...
    sqlx::query(
        "UPDATE responses SET error_kind = ?, error_detail = ?, elapsed_ms = ? \
         WHERE id = ?",
    )
    .bind(error_kind)
    .bind(error_detail)
...
/// Fetch the most recent per-debate response rows for a bot, newest first.
/// Used by `GET /bots/{id}/history` so an author can see the last N
/// rounds this bot participated in plus any recorded error classification.
pub async fn get_bot_history(
    ...
    sqlx::query_as::<_, ResponseRow>(
        "SELECT * FROM responses WHERE bot_id = ? \
