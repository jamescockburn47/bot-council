...
pub async fn get_any_bots_by_ids(pool: &SqlitePool, ids: &[String]) -> Result<Vec<BotRow>, sqlx::Error> {
    if ids.is_empty() { return Ok(Vec::new()); }
    let placeholders: String = ids.iter().map(|_| "?").collect::<Vec<_>>().join(",");
    let query = format!(
        "SELECT {BOT_COLUMNS} FROM bots WHERE id IN ({placeholders})"
    );
    let mut q = sqlx::query_as::<_, BotRow>(&query);
    for id in ids { q = q.bind(id); }
    q.fetch_all(pool).await
...
pub async fn list_debates(
    pool: &SqlitePool, status: Option<&str>, limit: i64, test_only: bool,
...
pub async fn get_bot_performance_aggregates(
    ...
    Ok(rows.into_iter().map(|row| (row.bot_id.clone(), row)).collect())
}
...
pub async fn get_bot_debate_summaries(
    ...
    sqlx::query_as::<_, BotDebateSummaryRow>(
        &sql
    )
    .bind(bot_id)
    .bind(limit)
