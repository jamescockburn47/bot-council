...
async fn add_admin_requires_clerk_format() {
    ...
    let req = common::admin_auth(
        Request::builder().method("POST").uri("/admins")
            .header("content-type", "application/json"),
    ).body(Body::from(r#"{"user_id":"not_a_clerk_id"}"#)).unwrap();
    let res = app.oneshot(req).await.unwrap();
...
async fn add_admin_happy_path() {
    ...
    let req = common::admin_auth(
        Request::builder().method("POST").uri("/admins")
            .header("content-type", "application/json"),
    ).body(Body::from(r#"{"user_id":"user_2abc123"}"#)).unwrap();
    let res = app.oneshot(req).await.unwrap();
    ...
    let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM admins WHERE user_id = ?")
        .bind("user_2abc123").fetch_one(&pool).await.unwrap();
    assert_eq!(count.0, 1);
}
...
async fn add_admin_idempotent() {
    ...
    for _ in 0..3 {
        let req = common::admin_auth(
            Request::builder().method("POST").uri("/admins")
                .header("content-type", "application/json"),
        ).body(Body::from(r#"{"user_id":"user_2same"}"#)).unwrap();
...
async fn remove_admin_happy_path() {
    ...
    let req = common::admin_auth(
        Request::builder().method("DELETE").uri("/admins/user_2tobeDeleted"),
    ).body(Body::empty()).unwrap();
    let res = app.oneshot(req).await.unwrap();
...
async fn remove_admin_not_found_returns_404() {
    let (app, _pool) = common::test_app().await;
    let req = common::admin_auth(
        Request::builder().method("DELETE").uri("/admins/user_2nonexistent"),
    ).body(Body::empty()).unwrap();
    let res = app.oneshot(req).await.unwrap();
...
async fn admin_endpoints_reject_unauthenticated() {
    let (app, _pool) = common::test_app().await;
    let req = Request::builder().method("GET").uri("/admins")
        .body(Body::empty()).unwrap();
    let res = app.oneshot(req).await.unwrap();
...
async fn list_users_marks_admin_status() {
    ...
    let regular = arr.iter().find(|u| u["user_id"] == "user_2regular").unwrap();
